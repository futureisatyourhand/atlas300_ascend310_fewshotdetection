...
# @File    : model.py
# Description :model and post-processing
# --------------------------------------
...
class Darknet(object):
    ...
    def decode(self,model_output):
        '''
        model_output: from darknet19 
        output_sizes:13*13
        '''
        H, W = self.output_sizes
        num_anchors = len(self.anchors) # 5
        ...
        with tf.variable_scope('xy_offset'): 
            # 
            xy_offset = tf.nn.sigmoid(model_output[:,:,:,0:2]) # 
        ...
        with tf.variable_scope('wh_offset'):
            wh_offset = tf.exp(model_output[:,:,:,2:4]) #
        ...
        # box(x,y,w,h) -> xmin=x-w/2 -> box(xmin,ymin,xmax,ymax)
        with tf.variable_scope('pred'):
        ...
        # 
        # x_cell=[[0,1,...,12],...,[0,1,...,12]]和y_cell=[[0,0,...,0],[1,...,1]...,[12,...,12]]
